      ...
      <div>
        <sp-radio-group label="Small" selected=${selectedPuzzle} name="selectedPuzzle">
          ...
          ${Object.keys(puzzles).map(puzzleName => {
            return html`<sp-radio style="border: 2px solid rgb(84, 84, 84); padding: 0.25em; border-radius: 30px; max-width: 300px; padding-left: 0.75em; padding-right: 0.75em; margin-bottom: 0.5em;" @change=${togglePuzzle} value=${puzzleName} size="s">${puzzleName}</sp-radio>`
          })}
        </sp-radio-group>
      </div>
      <div>
